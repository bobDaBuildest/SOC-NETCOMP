...
class DeviceConnectRequest(BaseModel):
    ...
    password: str
    port: int = 443
    device_type: str = "cisco_iosxe"  # cisco_iosxe, fortigate, paloalto
...
def connect_device(req: DeviceConnectRequest):
    ...
    if req.device_type == "cisco_iosxe":
        ...
        if connected:
            # Remove old entry for same host if exists
            global connected_devices
            connected_devices = [d for d in connected_devices if d["host"] != req.host]
            connected_devices.append(device)

        return device

    return {"error": f"Device type {req.device_type} not supported yet", "status": "error"}


@app.get("/api/devices")
def get_devices():
    """List all connected devices."""
    return {"devices": connected_devices, "total": len(connected_devices)}
