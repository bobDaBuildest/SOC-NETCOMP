...
class EventStream:
    ...
    def __init__(self):
        ...
        try:
            # Ensure mock/ is in path before importing DeviceCollector
            _mock_path = os.path.join(_HERE, "mock")
            _real_path = os.path.join(_HERE, "real")
            if _mock_path not in sys.path:
                sys.path.insert(0, _mock_path)
            if _real_path not in sys.path:
                sys.path.insert(0, _real_path)
            ...
            self.real = None
            self.has_real = False
            print(f"[INFO] Running in mock-only mode ({e})")
