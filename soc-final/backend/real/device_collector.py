...
class CiscoMerakiCollector:
    ...
    def get_security_events(
            self,
            network_id: Optional[str] = None) -> List[Dict]:
                    ...
                    normalize_event(
                        raw={
                            ...
                            "protocol": ev.get(
                                "protocol",
                                ""),
                            "action": "DENY" if "block" in ev.get(
                                "disposition",
                                "").lower() else "ALLOW",
    ...
    def start(self, callback: Optional[Callable] = None):
        ...
        def _listen():
            try:
                ...
                sock.settimeout(1.0)
                print(
                    f"🔊 Syslog receiver listening on {
                        self.host}:{
                        self.port}")
                while self.running:
                    try:
...
class DeviceCollector:
    ...
    def _check_connections(self):
        ...
        print(
            f"   Cisco IOS XE  : {
                '✅ Connected' if self.iosxe_ok else '❌ Unavailable — using mock'}")
        print(
            f"   Cisco Meraki  : {
                '✅ Connected' if self.meraki_ok else '❌ Unavailable — using mock'}")
